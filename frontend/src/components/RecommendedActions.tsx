...
import { Upload, MessageSquare, Briefcase, Search, Sparkles, Rocket } from 'lucide-react'
  ...
  return (
    <div className="card fade-in-up" style={{ marginBottom: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        {!hasProfile && (
          <button onClick={() => navigate('/onboarding/upload')} className="card card-hover" style={{ textAlign: 'left', cursor: 'pointer', border: '1.5px solid #e4e9ea', padding: 16 }}>
        ...
        {recCount === 0 && hasSkills && (
          <button onClick={() => navigate('/recommendations')} className="card card-hover" style={{ textAlign: 'left', cursor: 'pointer', border: '1.5px solid #e4e9ea', padding: 16 }}>
        ...
        {hasSkills && !hasExp && (
          <button onClick={() => navigate('/settings')} className="card card-hover" style={{ textAlign: 'left', cursor: 'pointer', border: '1.5px solid #e4e9ea', padding: 16 }}>
            <Sparkles size={18} color="#10b981" />
            <div style={{ fontWeight: 600, color: '#131f3d', marginTop: 8, fontSize: 14 }}>Refine preferences</div>
            <div style={{ fontSize: 12, color: '#76777e', marginTop: 4 }}>Tune your target roles</div>
          </button>
        )}
      </div>
    </div>
