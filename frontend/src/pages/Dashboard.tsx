...
import TopJobPreview from '../components/TopJobPreview'
import { useCurrentUser } from '../features/auth/useCurrentUser'
import { useStartInterview } from '../features/interview/useStartInterview'
...
export default function Dashboard() {
  ...
  const steps: Step[] = [
    { key: 'upload', label: 'Upload CV', desc: 'Import your resume', done: !!user?.isProfileCompleted, action: () => navigate('/onboarding/upload'), icon: Upload },
    { key: 'parse', label: 'AI Profile Analysis', desc: 'AI reads your background', done: skills.length > 0 || !!profile?.summary, action: () => navigate('/onboarding/upload'), icon: Brain },
    ...
    { key: 'skills', label: 'Skills Extraction', desc: 'AI maps your strengths', done: skills.length >= 5, action: () => navigate('/onboarding/upload'), icon: Target },
    { key: 'match', label: 'Job Matching', desc: 'Find roles that fit', done: (recommendations?.length ?? 0) > 0, action: () => navigate('/recommendations'), icon: Briefcase },
    { key: 'apply', label: 'Apply to Jobs', desc: 'Submit your first application', done: false, action: () => navigate('/recommendations'), icon: Rocket },
  ]
  ...
  return (
    <AppShell>
      <div ref={containerRef} style={{ maxWidth: 1120 }}>
        ...
        <div className="fade-in-up" style={{
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <button onClick={() => navigate('/onboarding/upload')} className="btn-primary" style={{ background: '#10b981' }}>
        ...
        <TopJobPreview jobs={topJobs} onSeeAll={() => navigate('/recommendations')} />
        ...
        {topJobs.length === 0 && skills.length === 0 && (
          <div className="card fade-in-up" style={{ textAlign: 'center', padding: 48 }}>
            ...
            <button onClick={() => navigate('/onboarding/upload')} className="btn-primary">
